...
const server = new Server(
  { name: 'styletoken-mcp-server', version: '0.1.0' },
  { capabilities: { tools: {} } }
)
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    ...
    throw new McpError(
      ...
      `Authentication failed: ${err instanceof Error ? err.message : 'Invalid token'}`
    )
  }
  ...
  try {
    ...
    switch (name) {
      ...
      case 'get_metadata': {
        ...
        if (!componentId) throw new McpError(ErrorCode.InvalidParams, 'component_id is required')
